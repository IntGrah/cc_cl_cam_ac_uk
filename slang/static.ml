let expected (e : Past.t) (expecting : string) (found : Type.t) =
  raise
    (Type.Type_error
       {
         loc = e.loc;
...
let rec elab (env : env) (e : Past.t) : Ast.t * Type.t =
  let elab_as env (e : Past.t) expecting =
    ...
    if found = expecting then
      ...
      expected e (Format.asprintf "%a" Type.pp_nice expecting) found
  in
  ...
  | UnaryOp (op, e) -> (
      ...
      let exactly (a : Type.t) (b : Type.t) =
        if t <> a then
          expected e (Format.asprintf "%a" Type.pp_nice a) t
        else
          b
      in
      match op with
  ...
  | BinaryOp (e1, op, e2) -> (
      ...
      let exactly a b c : Type.t =
        if t1 <> a then
          expected e1 (Format.asprintf "%a" Type.pp_nice a) t1
        else if t2 <> b then
          expected e2 (Format.asprintf "%a" Type.pp_nice b) t2
        else
          c
      in
      match op with
